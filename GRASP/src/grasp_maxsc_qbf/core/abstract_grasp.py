...
class AbstractGRASP(ABC, Generic[E]):
    ...
    def standard_constructive_heuristic(self, pop_in_construction=False) -> Solution[E]:
        ...
        while not self.constructive_stop_criteria():
            max_cost = float('-inf')
            min_cost = float('inf')
            self.value = self.obj_function.evaluate(self.sol)
            self.update_CL()
    ...
    def solve(self, time_limit, target_value) -> Solution[E]:
        ...
        for i in range(self.iterations):
            ...
            if self.best_sol.value < self.sol.value:  # Maximization
                self.best_sol = Solution(self.sol)
                if self.best_sol.value >= target_value:
                    time_to_target = time.time() - start_total_time_counter
                    target_reached = 1
                if self.verbose:
